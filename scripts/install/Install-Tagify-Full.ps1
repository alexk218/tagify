...
function Initialize-Logging {
    ...
    try {
        ...
        try {
            $latestSpicetifyRelease = Get-LatestSpicetifyReleaseInfo -TimeoutSec 5
            $latestSpicetifyVersion = $latestSpicetifyRelease.Version
...
function Test-SpicetifyState {
    ...
    try {
        ...
        try {
            $latestRelease = Get-LatestSpicetifyReleaseInfo -TimeoutSec 10
...
function Get-LatestSpicetifyReleaseInfo {
    [CmdletBinding()]
    param(
        [int]$TimeoutSec = 15
    )

    $latestRelease = Invoke-RestMethod -Uri 'https://api.github.com/repos/spicetify/cli/releases/latest' -TimeoutSec $TimeoutSec
    return @{
        Version = $latestRelease.tag_name -replace '^v', ''
        Body    = [string]$latestRelease.body
        Raw     = $latestRelease
    }
}
...
function Install-SpicetifyCore {
    ...
    try {
        ...
        $latestRelease = Get-LatestSpicetifyReleaseInfo
